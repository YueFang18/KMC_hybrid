...
int main( ) {
    ...
    double total_rate = 0.0, reaction = 0.0, time = 0.0, init_vol = 0.0, D_time = 0.0,d_time = 0.0,d_time_1 = 0.0,pre_d_time = 0.0,pre_d_time1 = 0.0,pre_d_time2 = 0.0;
...
//..............reading flags from user input...end..................//
    ...
    ofstream conv(out_dir + "1e9Hy1_conversion.out");
    ofstream molwt(out_dir+"1e9Hy1_molecularweight.out");
    ofstream fract(out_dir+"1e9Hy1_fractions.out");
    ofstream speciesout(out_dir+"1e9Hy1_species.out");
    ofstream species_con(out_dir+"1e9Hy1_spe_con.out");
    ofstream tim(out_dir+"1e9Hy1_time.out");
    ofstream sf(out_dir+"1e9Hy1_SF.out");
    ofstream m1m1(out_dir+"1e9Hy1_M1M1.out");
    ofstream polyinfo(out_dir+"1e9Hy1_polychain.out");
    ofstream polyinfomw(out_dir+"1e9Hy1_polychain_mw.out");
    ofstream aver(out_dir + "1e9Hy1_aver_spe3.out");

    ofstream ads_dis(out_dir+"1e9Hy1_ads_distribution.out");
    ofstream triad_dis(out_dir+"1e9Hy1_triad_distribution.out");
    ofstream eq_dyad(out_dir+"1e9Hy1_equal_dyad_point.out");
    ofstream ccd_count1(out_dir + "1e9Hy1_ccd_count1.out");
    ofstream sequence(out_dir + "1e9Hy1_chianinfo.out");
    ccd_count1 << "\t" << "ccd" << endl;
...
//calculate the SF at beginning with 2
        if ( scale_fac==1 && d_time>0.0005 && flag1 == 0) {

            ode(stiff_sys, system_jacobi,r,Rad,time);
            radical_ode=r[1]+r[4]+r[5]+r[6]+r[7]+r[8]+r[9];
            scale_fac =2/(Na * volume * radical_ode);
            sf<<"   "<<pre_add_spe2/pre_d_time2<<"   "<<scale_fac <<"   "<<radical_ode<<"    "<<"111   111"<<endl;
            pre_add_spe2 = 0;
            pre_d_time2 = 0;
            d_time=0;
            flag1 = 1;
...
//calculate the SF within 1000s(average), the larger of d_time1, the longer of the calculation
        if (d_time>0.1 && pre_d_time2!=0 && fl_aver==0){

            if (fl_aver==0){
                aver_spe3 = pre_add_spe2/pre_d_time2;
                aver<<aver_spe3<<endl;
                fl_aver=1;
            }
                ode(stiff_sys, system_jacobi,r,Rad,time);
                ...
                sf<<"   "<<pre_add_spe2/pre_d_time2<<"   "<<scale_fac <<"   "<<radical_ode<<"    "<<"222   222"<<endl;
...
//setting for re-update with 2
        ...
        if (D_time >= 100.0) {
//            sf<<pre_add_spe2/pre_d_time2<<"   "<<scale_fac<<"   "<<all_species/(Na * volume) <<"   "<<radical_ode<<endl;
