...
//..............reading flags from user input...end..................//
    ...
    ofstream conv(out_dir + "1e8KMC_conversion.out");
    ofstream molwt(out_dir+"1e8KMC_molecularweight.out");
    ofstream fract(out_dir+"1e8KMC_fractions.out");
    ofstream speciesout(out_dir+"1e8KMC_species.out");
    ofstream species_con(out_dir+"1e8KMC_spe_con.out");
    ofstream tim(out_dir+"1e8KMC_time.out");
    ofstream sf(out_dir+"1e8KMC_SF.out");
    ofstream m1m1(out_dir+"1e8KMC_M1M1.out");
    ofstream polyinfo(out_dir+"1e8KMC_polychain.out");
    ofstream polyinfomw(out_dir+"1e8KMC_polychain_mw.out");
    ofstream aver(out_dir + "1e8KMC_aver_spe3.out");

    ofstream ads_dis(out_dir+"1e8KMC_ads_distribution.out");
    ofstream triad_dis(out_dir+"1e8KMC_triad_distribution.out");
    ofstream eq_dyad(out_dir+"1e8KMC_equal_dyad_point.out");
    ofstream ccd_count1(out_dir + "1e8KMC_ccd_count1.out");
    ofstream sequence(out_dir + "1e8KMC_chianinfo.out");
    ccd_count1 << "\t" << "ccd" << endl;
...
//        cout<<"111          111"<<"    "<<rand_11<<"    "<<rand_22<<"    "<<tau<<"  "<<total_rate<<endl;
//calculate the SF at beginning with 2
//        if ( scale_fac == 1 && d_time > 0.005 && flag1 == 0) {
//
...
//        }
//
//        if (Aver_d_time > 0.01 && Aver_add_spe/ Aver_d_time>1 && fl_aver == 0){
//
//            if (fl_aver==0){
...
//            sf<<"   "<<Aver_add_spe/Aver_d_time<<"   "<<scale_fac <<"   "<<radical_ode<<"    "<<r[2]<<"    "<<r[3]<<"    "<<"222   222"<<"    "<<rand_11<<"    "<<rand_22<<endl;
//
//        }
//
//
