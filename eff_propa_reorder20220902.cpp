...
int main( ) {
    ...
    double total_rate = 0.0, reaction = 0.0, time = 0.0, init_vol = 0.0, D_time = 0.0,d_time = 0.0,d_time1 = 0.0,d_time2 = 0.0,pre_d_time = 0.0,pre_d_time1 = 0.0,pre_d_time2 = 0.0;
    ...
    double radical_ode = 0.0, radical_kmc = 0.0,pre_add_spe=0.0,pre_add_spe1=0.0,pre_add_spe2=0.0;
    double x[11]={0.0};
...
//..............reading flags from user input...end..................//
    ...
    ofstream conv(out_dir + "1e10KMC_conversion.out");
    ofstream molwt(out_dir+"1e10KMC_molecularweight.out");
    ofstream fract(out_dir+"1e10KMC_fractions.out");
    ofstream speciesout(out_dir+"1e10KMC_species.out");
    ofstream species_con(out_dir+"1e10KMC_spe_con.out");
    ofstream tim(out_dir+"1e10KMC_time.out");
    ofstream sf(out_dir+"1e10KMC_SF.out");
    ofstream m1m1(out_dir+"1e10KMC_M1M1.out");
    ofstream polyinfo(out_dir+"1e10KMC_polychain.out");
    ofstream polyinfomw(out_dir+"1e10KMC_polychain_mw.out");

    ofstream ads_dis(out_dir+"1e10KMC_ads_distribution.out");
    ofstream triad_dis(out_dir+"1e10KMC_triad_distribution.out");
    ofstream eq_dyad(out_dir+"1e10KMC_equal_dyad_point.out");
    ofstream ccd_count1(out_dir + "ccd_count1.out");
    ofstream sequence(out_dir + "1e10KMC_chianinfo.out");
    ccd_count1 << "\t" << "ccd" << endl;
    ...
    rate[ 9  ]=	1/scale_fac/scale_fac	*	c[ 16 ]*2/(Na*volume)*species[ 6 ]*species[ 7 ];
    rate[ 10 ]=	1/scale_fac/scale_fac	*	c[ 17 ]*2/(Na*volume)*species[ 6 ]*species[ 8 ];
    rate[ 11 ]=	1/scale_fac/scale_fac	*	c[ 18 ]*2/(Na*volume)*species[ 6 ]*species[ 9 ];
    rate[ 12 ]=	1/scale_fac/scale_fac	*	c[ 19 ]*2/(Na*volume)*species[ 7 ]*(species[ 7 ]-1)/2;
    rate[ 13 ]=	1/scale_fac/scale_fac	*	c[ 20 ]*2/(Na*volume)*species[ 7 ]*species[ 8 ];
    rate[ 14 ]=	1/scale_fac/scale_fac	*	c[ 21 ]*2/(Na*volume)*species[ 7 ]*species[ 9 ];
    rate[ 15 ]=	1/scale_fac/scale_fac	*	c[ 22 ]*2/(Na*volume)*species[ 8 ]*(species[ 8 ]-1)/2;
    rate[ 16 ]=	1/scale_fac/scale_fac	*	c[ 23 ]*2/(Na*volume)*species[ 8 ]*species[ 9 ];
    ...
    rate[ 19 ]=	1/scale_fac/scale_fac	*	c[ 26 ]*2/(Na*volume)*species[ 4 ]*species[ 5 ];
    rate[ 20 ]=	1/scale_fac/scale_fac	*	c[ 27 ]*2/(Na*volume)*species[ 5 ]*(species[ 5 ]-1)/2;
    rate[ 21 ]=	1/scale_fac/scale_fac	*	c[ 28 ]*2/(Na*volume)*species[ 4 ]*species[ 6 ];
    rate[ 22 ]=	1/scale_fac/scale_fac	*	c[ 29 ]*2/(Na*volume)*species[ 4 ]*species[ 7 ];
    rate[ 23 ]=	1/scale_fac/scale_fac	*	c[ 30 ]*2/(Na*volume)*species[ 4 ]*species[ 8 ];
    rate[ 24 ]=	1/scale_fac/scale_fac	*	c[ 31 ]*2/(Na*volume)*species[ 4 ]*species[ 9 ];
    rate[ 25 ]=	1/scale_fac/scale_fac	*	c[ 32 ]*2/(Na*volume)*species[ 5 ]*species[ 6 ];
    rate[ 26 ]=	1/scale_fac/scale_fac	*	c[ 33 ]*2/(Na*volume)*species[ 5 ]*species[ 7 ];
    rate[ 27 ]=	1/scale_fac/scale_fac	*	c[ 34 ]*2/(Na*volume)*species[ 5 ]*species[ 8 ];
    rate[ 28 ]=	1/scale_fac/scale_fac	*	c[ 35 ]*2/(Na*volume)*species[ 5 ]*species[ 9 ];
    rate[ 29 ]=	1/scale_fac/scale_fac	*	c[ 36 ]*2/(Na*volume)*species[ 6 ]*(species[ 6 ]-1)/2;
    rate[ 30 ]=	1/scale_fac/scale_fac	*	c[ 37 ]*2/(Na*volume)*species[ 6 ]*species[ 7 ];
    rate[ 31 ]=	1/scale_fac/scale_fac	*	c[ 38 ]*2/(Na*volume)*species[ 6 ]*species[ 8 ];
    rate[ 32 ]=	1/scale_fac/scale_fac	*	c[ 39 ]*2/(Na*volume)*species[ 6 ]*species[ 9 ];
    rate[ 33 ]=	1/scale_fac/scale_fac	*	c[ 40 ]*2/(Na*volume)*species[ 7 ]*(species[ 7 ]-1)/2;
    rate[ 34 ]=	1/scale_fac/scale_fac	*	c[ 41 ]*2/(Na*volume)*species[ 7 ]*species[ 8 ];
    rate[ 35 ]=	1/scale_fac/scale_fac	*	c[ 42 ]*2/(Na*volume)*species[ 7 ]*species[ 9 ];
    rate[ 36 ]=	1/scale_fac/scale_fac	*	c[ 43 ]*2/(Na*volume)*species[ 8 ]*(species[ 8 ]-1)/2;
    rate[ 37 ]=	1/scale_fac/scale_fac	*	c[ 44 ]*2/(Na*volume)*species[ 8 ]*species[ 9 ];
    ...
    rate[ 40 ]=	1/scale_fac/scale_fac	*	c[ 47 ]*2/(Na*volume)*species[ 4 ]*species[ 5 ];
    rate[ 41 ]=	1/scale_fac/scale_fac	*	c[ 48 ]*2/(Na*volume)*species[ 5 ]*(species[ 5 ]-1)/2;
    rate[ 42 ]=	1/scale_fac/scale_fac	*	c[ 49 ]*2/(Na*volume)*species[ 4 ]*species[ 6 ];
    rate[ 43 ]=	1/scale_fac/scale_fac	*	c[ 50 ]*2/(Na*volume)*species[ 4 ]*species[ 7 ];
    rate[ 44 ]=	1/scale_fac/scale_fac	*	c[ 51 ]*2/(Na*volume)*species[ 4 ]*species[ 8 ];
    rate[ 45 ]=	1/scale_fac/scale_fac	*	c[ 52 ]*2/(Na*volume)*species[ 4 ]*species[ 9 ];
    rate[ 46 ]=	1/scale_fac/scale_fac	*	c[ 53 ]*2/(Na*volume)*species[ 5 ]*species[ 6 ];
    rate[ 47 ]=	1/scale_fac/scale_fac	*	c[ 54 ]*2/(Na*volume)*species[ 5 ]*species[ 7 ];
    rate[ 48 ]=	1/scale_fac/scale_fac	*	c[ 55 ]*2/(Na*volume)*species[ 5 ]*species[ 8 ];
    rate[ 49 ]=	1/scale_fac/scale_fac	*	c[ 56 ]*2/(Na*volume)*species[ 5 ]*species[ 9 ];
    ...
    while (time <= time_limit) {
        ...
        if (all_species != 0.0){
            pre_add_spe2 += (all_species) * tau;
            pre_d_time2 += tau;
        }

//calculate the SF at beginning with 2
//        if (scale_fac==1 or d_time < 0.001) {
//
//            ode(stiff_sys, system_jacobi,Rad,r,time);
...
//        }
//
////calculate the SF within 1000s(average), the larger of d_time1, the longer of the calculation
//        if (d_time1>10000 ){
//            ode(stiff_sys, system_jacobi,Rad,r,time);
//            radical_ode=r[1]+r[4]+r[5]+r[6]+r[7]+r[8]+r[9];
//            scale_fac =pre_add_spe2/pre_d_time2/(Na * volume)/radical_ode;
//            pre_add_spe2 = 0;
//            pre_d_time2 = 0;
//            d_time1=0;
//        }

//setting for re-update with 2
        if ( d_time >20000 ){
            d_time=0.0;
            cout<<"111          111"<<endl;
        }
        ...
        if (D_time >= 10.0) {
            ...
            sf<<pre_add_spe2/pre_d_time2<<"   "<<scale_fac<<endl;
...
//         molecular_weight(time, mol_wt, Mn, Mw, num_M, poly); //function for calculating molecular weight
                molecular_weight(mol_wt,
                                 ...
                                 Mw,
                                 num_M,
                                 poly.chain1); //function for calculating molecular weight
...
//        cout<<scale_fac<<endl;
        ...
        if (dep_graph[reaction_index][9]==1) rate[9]=1/scale_fac/scale_fac*c[16]*2/(Na*volume)*species[6]*species[7];
        if (dep_graph[reaction_index][10]==1) rate[10]=1/scale_fac/scale_fac*c[17]*2/(Na*volume)*species[6]*species[8];
        if (dep_graph[reaction_index][11]==1) rate[11]=1/scale_fac/scale_fac*c[18]*2/(Na*volume)*species[6]*species[9];
        if (dep_graph[reaction_index][12]==1) rate[12]=1/scale_fac/scale_fac*c[19]*2/(Na*volume)*species[7]*(species[7]-1)/2;
        if (dep_graph[reaction_index][13]==1) rate[13]=1/scale_fac/scale_fac*c[20]*2/(Na*volume)*species[7]*species[8];
        if (dep_graph[reaction_index][14]==1) rate[14]=1/scale_fac/scale_fac*c[21]*2/(Na*volume)*species[7]*species[9];
        if (dep_graph[reaction_index][15]==1) rate[15]=1/scale_fac/scale_fac*c[22]*2/(Na*volume)*species[8]*(species[8]-1)/2;
        if (dep_graph[reaction_index][16]==1) rate[16]=1/scale_fac/scale_fac*c[23]*2/(Na*volume)*species[8]*species[9];
        ...
        if (dep_graph[reaction_index][19]==1) rate[19]=1/scale_fac/scale_fac*c[26]*2/(Na*volume)*species[4]*species[5];
        if (dep_graph[reaction_index][20]==1) rate[20]=1/scale_fac/scale_fac*c[27]*2/(Na*volume)*species[5]*(species[5]-1)/2;
        if (dep_graph[reaction_index][21]==1) rate[21]=1/scale_fac/scale_fac*c[28]*2/(Na*volume)*species[4]*species[6];
        if (dep_graph[reaction_index][22]==1) rate[22]=1/scale_fac/scale_fac*c[29]*2/(Na*volume)*species[4]*species[7];
        if (dep_graph[reaction_index][23]==1) rate[23]=1/scale_fac/scale_fac*c[30]*2/(Na*volume)*species[4]*species[8];
        if (dep_graph[reaction_index][24]==1) rate[24]=1/scale_fac/scale_fac*c[31]*2/(Na*volume)*species[4]*species[9];
        if (dep_graph[reaction_index][25]==1) rate[25]=1/scale_fac/scale_fac*c[32]*2/(Na*volume)*species[5]*species[6];
        if (dep_graph[reaction_index][26]==1) rate[26]=1/scale_fac/scale_fac*c[33]*2/(Na*volume)*species[5]*species[7];
        if (dep_graph[reaction_index][27]==1) rate[27]=1/scale_fac/scale_fac*c[34]*2/(Na*volume)*species[5]*species[8];
        if (dep_graph[reaction_index][28]==1) rate[28]=1/scale_fac/scale_fac*c[35]*2/(Na*volume)*species[5]*species[9];
        if (dep_graph[reaction_index][29]==1) rate[29]=1/scale_fac/scale_fac*c[36]*2/(Na*volume)*species[6]*(species[6]-1)/2;
        if (dep_graph[reaction_index][30]==1) rate[30]=1/scale_fac/scale_fac*c[37]*2/(Na*volume)*species[6]*species[7];
        if (dep_graph[reaction_index][31]==1) rate[31]=1/scale_fac/scale_fac*c[38]*2/(Na*volume)*species[6]*species[8];
        if (dep_graph[reaction_index][32]==1) rate[32]=1/scale_fac/scale_fac*c[39]*2/(Na*volume)*species[6]*species[9];
        if (dep_graph[reaction_index][33]==1) rate[33]=1/scale_fac/scale_fac*c[40]*2/(Na*volume)*species[7]*(species[7]-1)/2;
        if (dep_graph[reaction_index][34]==1) rate[34]=1/scale_fac/scale_fac*c[41]*2/(Na*volume)*species[7]*species[8];
        if (dep_graph[reaction_index][35]==1) rate[35]=1/scale_fac/scale_fac*c[42]*2/(Na*volume)*species[7]*species[9];
        if (dep_graph[reaction_index][36]==1) rate[36]=1/scale_fac/scale_fac*c[43]*2/(Na*volume)*species[8]*(species[8]-1)/2;
        if (dep_graph[reaction_index][37]==1) rate[37]=1/scale_fac/scale_fac*c[44]*2/(Na*volume)*species[8]*species[9];
        ...
        if (dep_graph[reaction_index][40]==1) rate[40]=1/scale_fac/scale_fac*c[47]*2/(Na*volume)*species[4]*species[5];
        if (dep_graph[reaction_index][41]==1) rate[41]=1/scale_fac/scale_fac*c[48]*2/(Na*volume)*species[5]*(species[5]-1)/2;
        if (dep_graph[reaction_index][42]==1) rate[42]=1/scale_fac/scale_fac*c[49]*2/(Na*volume)*species[4]*species[6];
        if (dep_graph[reaction_index][43]==1) rate[43]=1/scale_fac/scale_fac*c[50]*2/(Na*volume)*species[4]*species[7];
        if (dep_graph[reaction_index][44]==1) rate[44]=1/scale_fac/scale_fac*c[51]*2/(Na*volume)*species[4]*species[8];
        if (dep_graph[reaction_index][45]==1) rate[45]=1/scale_fac/scale_fac*c[52]*2/(Na*volume)*species[4]*species[9];
        if (dep_graph[reaction_index][46]==1) rate[46]=1/scale_fac/scale_fac*c[53]*2/(Na*volume)*species[5]*species[6];
        if (dep_graph[reaction_index][47]==1) rate[47]=1/scale_fac/scale_fac*c[54]*2/(Na*volume)*species[5]*species[7];
        if (dep_graph[reaction_index][48]==1) rate[48]=1/scale_fac/scale_fac*c[55]*2/(Na*volume)*species[5]*species[8];
        if (dep_graph[reaction_index][49]==1) rate[49]=1/scale_fac/scale_fac*c[56]*2/(Na*volume)*species[5]*species[9];
